...
def process(audio_path,dict):
    ...
    for path in tqdm(Path(audio_path).rglob('*.wav')):
...
def preprocess():
    ...
    with open(os.path.join(manifest_path_ub, "tr.csv"), "w") as f:
        f.write(unb_train_csv_output)
    # with open(os.path.join(manifest_path_b,"tr.csv"),"w") as f:
...
if __name__ == "__main__":
    audio_path = "/defaultShare/audioset"
    save_path = "/20A021/audioset"

    csv_path = os.path.join(save_path, "csv")

    # manifest_path_b = os.path.join(save_path,"manifest_b")
    manifest_path_ub = os.path.join(save_path, "manifest_ub")

    # lmdb_path_b = os.path.join(save_path,"lmdb_b")
    lmdb_path_ub = os.path.join(save_path, "lmdb_ub")

    # preprocess()

    folder2lmdb(manifest_path_ub,"train",lmdb_path_ub, max_num=400000, write_frequency=10000)
