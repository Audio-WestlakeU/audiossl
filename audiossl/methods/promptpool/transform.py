...
class MixATSTTrainTransform:
    def __init__(self,sr=16000,aug_tea=False,aug_stu=True,mask_ratio=0.65,min_mask_len=2,mask_len=5,mask_type="block",anchor_len=6.,patch_h=64,patch_w=4,**kwargs):
        ...
        self.positive_transform3 = transforms.Compose(
                            [
                            Mixup(),
                            RandomResizeCrop((1,1.5)),
                            ]
                            )

        self.positive_transform4 = transforms.Compose(
                            [
                            Mixup(),
                            RandomResizeCrop((1,1.5)),
                            ]
    ...
    def __call__(self,input):
        ...
        crop_positive3=self.positivecrop(input)
        crop_positive4=self.positivecrop(input)
        ...
        crops.append(F.pad(self.positive_transform3(crop_positive3),
                          (0,int((self.max_positive_len*16000)//160-int(positive_len*16000)//160))))
        lengths.append(int(positive_len*16000)//160+1)

        crops.append(F.pad(self.positive_transform4(crop_positive4),
                          (0,int((self.max_positive_len*16000)//160-int(positive_len*16000)//160))))
        lengths.append(int(positive_len*16000)//160+1)
